# -*- coding: utf-8 -*-
"""
Created on Thu Jul 21 22:08:40 2022

@author: gldz
"""


from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtWidgets import QMessageBox,QWidget,QApplication
from faces_input_frame import Ui_Dialog
from faces_register import Ui_Dialog2
from v1 import AIGUI
from lunbo import MyClass


import os
 
 
class Ui_Form(QWidget):  #将object改为QWidget，才能弹出消息对话框
    def __init__(self):
        super(Ui_Form,self).__init__() #用户添加代码
    def setupUi(self, Form):
        self.form=Form  #用户添加代码
        Form.setObjectName("Form")
        Form.setMinimumSize(QtCore.QSize(329, 230))
        Form.setMaximumSize(QtCore.QSize(400, 230))
        Form.setStyleSheet("")
        Form.setStyleSheet("background-image:url(icon/bgp_w.bmp)")
        self.label = QtWidgets.QLabel(Form)
        self.label.setGeometry(QtCore.QRect(63, 43, 64, 16))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.label.setFont(font)
        self.label.setObjectName("label")
        self.label_2 = QtWidgets.QLabel(Form)
        self.label_2.setGeometry(QtCore.QRect(63, 80, 48, 16))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.label_2.setFont(font)
        self.label_2.setObjectName("label_2")
        self.lineEdit_2 = QtWidgets.QLineEdit(Form)
        with open('1.QSS', 'r') as f:
            qss1 = f.read()
        self.lineEdit_2.setStyleSheet(qss1)
        self.lineEdit_2.setGeometry(QtCore.QRect(100, 80, 163, 20))
        self.lineEdit_2.setEchoMode(QtWidgets.QLineEdit.Password)
        self.lineEdit_2.setCursorPosition(0)
        self.lineEdit_2.setObjectName("lineEdit_2")
        self.pushButton = QtWidgets.QPushButton(Form)
        self.pushButton.setStyleSheet("QPushButton{border-image: url(icon/button_1.bmp)}") #绑定图片
        self.pushButton.setGeometry(QtCore.QRect(70, 150, 75, 23))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.pushButton.setFont(font)
        self.pushButton.setObjectName("pushButton")
        self.pushButton_2 = QtWidgets.QPushButton(Form)
        self.pushButton_2.setStyleSheet("QPushButton{border-image: url(icon/button_1.bmp)}") #绑定图片
        self.pushButton_2.setGeometry(QtCore.QRect(170, 150, 75, 23))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.pushButton_2.setFont(font)
        self.pushButton_2.setObjectName("pushButton_2")
        self.checkBox = QtWidgets.QCheckBox(Form)
        self.checkBox.setGeometry(QtCore.QRect(63, 110, 151, 20))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.checkBox.setFont(font)
        self.checkBox.setObjectName("checkBox")
        self.lineEdit_3 = QtWidgets.QLineEdit(Form)
        self.lineEdit_3.setStyleSheet(qss1)
        self.lineEdit_3.setGeometry(QtCore.QRect(100, 40, 163, 20))
        self.lineEdit_3.setObjectName("lineEdit_3")
        self.pushButton_face_pass = QtWidgets.QPushButton(Form)
        self.pushButton_face_pass.setStyleSheet("QPushButton{border-image: url(icon/button_1.bmp)}") #绑定图片
        self.pushButton_face_pass.setGeometry(QtCore.QRect(279, 90, 104, 41))
        self.pushButton_face_pass.setMaximumSize(QtCore.QSize(16777215, 16777213))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.pushButton_face_pass.setFont(font)
        self.pushButton_face_pass.setObjectName("pushButton_face_pass")
        
        self.pushButton_face_input = QtWidgets.QPushButton(Form)
        self.pushButton_face_input.setStyleSheet("QPushButton{border-image: url(icon/button_1.bmp)}") #绑定图片
        self.pushButton_face_input.setGeometry(QtCore.QRect(280, 31, 104, 41))
        self.pushButton_face_input.setMaximumSize(QtCore.QSize(16777215, 16777213))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.pushButton_face_input.setFont(font)
        self.pushButton_face_input.setObjectName("pushButton_face_input")
        
        self.pushButton_register = QtWidgets.QPushButton(Form)
        self.pushButton_register.setStyleSheet("QPushButton{border-image: url(icon/button_1.bmp)}") #绑定图片
        self.pushButton_register.setGeometry(QtCore.QRect(279, 150, 104, 41))
        self.pushButton_register.setMaximumSize(QtCore.QSize(16777215, 16777213))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.pushButton_register.setFont(font)
        self.pushButton_register.setObjectName("pushButton_register")
        
        self.icon1 = QtWidgets.QLabel(Form)
        self.icon1.setGeometry(QtCore.QRect(30, 40, 25, 25))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.icon1.setStyleSheet("border-image: url(icon/用户_user.png);")
        self.icon1.setObjectName("icon1")
        
        self.icon2 = QtWidgets.QLabel(Form)
        self.icon2.setGeometry(QtCore.QRect(30, 75, 25, 25))
        font = QtGui.QFont()
        font.setPointSize(8)
        self.icon2.setStyleSheet("border-image: url(icon/钥匙_key.png);")
        self.icon2.setObjectName("icon2")
        
        # self.icon3 = QtWidgets.QLabel(Form)
        # self.icon3.setGeometry(QtCore.QRect(300, 75, 25, 25))
        # self.icon3.setStyleSheet("border-image: url(icon/添加人群_people-plus.png);")
        # self.icon3.setObjectName("icon3")
        
        # self.icon4 = QtWidgets.QLabel(Form)
        # self.icon4.setGeometry(QtCore.QRect(300, 75, 25, 25))
        # self.icon4.setStyleSheet("border-image: url(icon/添加人群_people-plus-one.png);")
        # self.icon4.setObjectName("icon4")
        
        # self.icon5 = QtWidgets.QLabel(Form)
        # self.icon5.setGeometry(QtCore.QRect(300, 75, 25, 25))
        # self.icon5.setStyleSheet("border-image: url(icon/用户定位_user-positioning.png);")
        # self.icon5.setObjectName("icon5")
 
        self.retranslateUi(Form)
        self.pushButton.clicked.connect(self.close)
        self.pushButton_2.clicked.connect(self.open)
        self.pushButton_face_input.clicked.connect(self.faceinput)
        self.pushButton_face_pass.clicked.connect(self.facepass)
        self.pushButton_register.clicked.connect(self.register)
 
    def retranslateUi(self, Form):
        _translate = QtCore.QCoreApplication.translate
        Form.setWindowTitle(_translate("Form", "植物识别"))
        self.label.setText(_translate("Form", "用户名："))
        self.label_2.setText(_translate("Form", "密码："))
        self.pushButton.setText(_translate("Form", "取消"))
        self.pushButton_2.setText(_translate("Form", "确认"))
        self.checkBox.setText(_translate("Form", "记住用户名和密码"))
        self.pushButton_face_pass.setText(_translate("Form", "人脸登录"))
        self.pushButton_face_input.setText(_translate("Form", "人脸注册"))
        self.pushButton_register.setText(_translate("Form", "注册账号"))
 
    def open(self):
        #--------判断用户是否存在--------------
        fl = open('user_names.txt', 'r+')
        pre_name = fl.read()
        fl.close()
        names = pre_name.split(',')
        names = [x.strip() for x in names if x.strip() != '']
        
        password=[]
        for i in range(1,len(names),2):
            password.append(names[i])
            
        nam=[]
        for i in range(0,len(names),2):
            nam.append(names[i])
        
        if self.lineEdit_3.text() in nam:
            # fl = open('password.txt', 'r+')
            # password= fl.read()
            x=nam.index(self.lineEdit_3.text())
            if self.lineEdit_2.text() == password[x]:
                reply=QMessageBox.information(self,'提示','登录成功',QMessageBox.Close)
                
                self.form.hide()
                Form1=QtWidgets.QDialog()
                ui=AIGUI()
                ui.initUI(Form1)
                Form1.show()
                Form1.exec_()
                self.form.show()   #子窗口关闭后，主窗口显示
                
            else:
                reply=QMessageBox.information(self,'提示','密码错误',QMessageBox.Close)
 
 
        else:
            reply=QMessageBox.information(self,'提示','用户不存在',QMessageBox.Close)
 
 
    def close(self, event):
        self.close()
    
    def faceinput(self,event):
        self.form.hide()
        Form1=QtWidgets.QDialog()
        ui=Ui_Dialog()
        ui.setupUi(Form1)
        Form1.show()
        Form1.exec_()
        self.form.show()   #子窗口关闭后，主窗口显示

 
    def facepass(self,event):
        import face_recognize
        get_name=face_recognize.recognize_face()#返回识别的人名
        if get_name=="unknown":
            reply = QMessageBox.information(self, '提示', '人脸识别失败', QMessageBox.Close)
        else:
            reply = QMessageBox.information(self, '提示', "欢迎您："+get_name, QMessageBox.Ok)
            print("登录成功")
            
            self.form.hide()
            Form1=QtWidgets.QDialog()
            ui=AIGUI()
            ui.initUI(Form1)
            Form1.show()
            Form1.exec_()
            self.form.show()   #子窗口关闭后，主窗口显示
            
            
    def register(self,event):
        self.form.hide()
        Form1=QtWidgets.QDialog()
        ui=Ui_Dialog2()
        ui.setupUi(Form1)
        Form1.show()
        Form1.exec_()
        self.form.show()   #子窗口关闭后，主窗口显示
 
 
if __name__=="__main__":
    import sys
    app=QtWidgets.QApplication(sys.argv)
    widget=QtWidgets.QWidget()
    ui=Ui_Form()
    ui.setupUi(widget)
    widget.show()
    sys.exit(app.exec_())